...
test('shared corestores will share write capabilities', async t => {
  ...
  function onready () {
    const drive1 = create({ dwebstore: store, namespace: 'ns1' })
    const drive2 = create({ dwebstore: store, namespace: 'ns2' })
...
test('mount replication between hyperdrives, multiple, nested mounts', async t => {
  ...
  function createMountee () {
    ...
    return new Promise(resolve => {
      drive1.ready(err => {
        t.error(err, 'no error')
        drive2 = create({ dwebstore: store, namespace: 'ns2' })
        drive3 = create({ dwebstore: store, namespace: 'ns3' })
        drive2.ready(err => {
          t.error(err, 'no error')
          drive3.ready(err => {
            t.error(err, 'no error')
...
test('update does not clear the mount', function (t) {
  ...
  const other = ddrive(drive.dwebstore, null, { namespace: 'test' })
