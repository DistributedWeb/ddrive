...
module.exports = (...args) => new DDrive(...args)
...
class DDrive extends Nanoresource {
  constructor (storage, key, opts) {
    ...
    this.dwebstore = defaultCorestore(storage, {
      ...opts,
    ...
    if (this.dwebstore !== storage) this.dwebstore.on('error', err => this.emit('error', err))
    if (opts.namespace) {
      this.dwebstore = this.dwebstore.namespace(opts.namespace)
    }
  ...
  _open (cb) {
    const self = this
    return this.dwebstore.ready(err => {
      if (err) return cb(err)
      this.metadata = this.dwebstore.default(this._metadataOpts)
      this.db = this.db || new MountableHypertrie(this.dwebstore, this.key, {
        feed: this.metadata,
        sparse: this.sparseMetadata,
  ...
  _contentStateFromKey (publicKey, cb) {
    ...
    try {
      var feed = this.dwebstore.get(contentOpts)
    } catch (err) {
      return cb(err)
    }
  ...
  replicate (isInitiator, opts) {
    ...
    this.ready(err => {
      if (err) return stream.destroy(err)
      this.dwebstore.replicate(isInitiator, { ...opts, stream })
    })
    return stream
  }
  ...
  checkout (version, opts) {
    ...
    return new DDrive(this.dwebstore, this.key, opts)
  }
